  ...
  calculaSubtotal() {
    this._subtotal = reservas.reduce(
      (acumulado, { tipoHabitacion, pax, noches }) =>
        acumulado +
        (this.calcularPrecioHabitacion(tipoHabitacion) * noches +
          this.suplementoPersonaAdicional(pax)),
      0
    );
  }
  ...
  calculaSubtotal() {
    this._subtotal = reservas.reduce(
      (acumulado, { pax, noches }) =>
        acumulado +
        (this.precioHabitacion * noches + this.suplementoPersonaAdicional(pax)),
      0
    );
  }
...
console.log(
  "Subtotal para un particular: ",
  reservaTouroperador.subtotal + "€"
);
